...
func newConsumer(
	...
	if logger != nil {
		logger = logger.WithModule("queue.kafka")
	}
...
func (c *consumer) Consume(ctx context.Context, handler queue.DeliveryHandler) error {
	if handler == nil {
...
func (c *consumer) consumeClient(
	...
	defer func() {
		...
		if leaveErr != nil && c.logger != nil {
			c.logger.Warnw("msg", "consumeClient | Kafka leave group failed", "queue.consumer", instance, "error", leaveErr)
		}
	}()

	if c.logger != nil {
		c.logger.Debugw(
			"msg", "Kafka queue consumer ready",
			"queue.destination", c.config.Topic,
