...
func TestProcessFetchesPreservesMultiplePartitionsAndBrokers(t *testing.T) {
	...
	committer := newRecordingCommitter(nil)
	var ids []string
	if err := processFetches(context.Background(), committer, fetches, func(_ context.Context, d queue.Delivery) error { ids = append(ids, d.Message.ID); return nil }); err != nil {
		t.Fatal(err)
	}
...
func BenchmarkExternalKafkaBatch(b *testing.B) {
	...
	for _, batch := range []int{1, 32, 128} {
		b.Run(fmt.Sprint(batch), func(b *testing.B) {
			...
			defer cleanup()
			messages := make([]*queue.Message, b.N*batch+batch)
			...
			for i := range messages {
				...
				messages[i] = &queue.Message{Body: body}
			}
			if err := producer.PublishBatch(ctx, messages); err != nil {
				b.Fatal(err)
			}
			client, err := factory.NewConsumerClient("external", consumerClientOptions(ConsumerConfig{Topic: topic, Group: topic, StartPosition: queue.StartEarliest}, topic)...)
			if err != nil {
				b.Fatal(err)
			}
