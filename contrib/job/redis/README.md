...
完整的构造、错误处理与 Bootstrap 登记示例统一见 [Job 使用说明](../../../pkg/job/README.md)。顺序是：借用 Redis 连接构造 coordinator，在 Spec 注入它并为 Cron 设置 `SkipIfDistributedRunning` 或 `DelayIfDistributedRunning`，然后创建 Manager。单独调用 `Spec.Coordinator` 不会把 `SkipIfRunning` 升级成跨进程互斥；Manager 构造后也不会重新读取 Spec。

```mermaid
flowchart TD
    A([开始]) --> B[NewLockCoordinator 借用具名 Redis 连接]
    B -- 失败 --> E([返回错误])
    B -- 成功 --> C[Spec 注入 coordinator 并显式选择分布式 Cron 策略]
    C --> D[NewManager 固定声明]
    D -- 校验失败 --> E
