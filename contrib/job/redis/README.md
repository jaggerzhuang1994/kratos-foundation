...
Job 锁实现由业务/Wire 显式选择；这里没有全局驱动注册表，也不会根据 `job.lock.driver` 自动分发。需要其他协调方案时，业务可以直接构造实现 `job.ConcurrencyCoordinator` 的公共组件并通过构造参数交给 `job.NewManager` 或 `bootstrap.NewComponentsBootstrap`。
