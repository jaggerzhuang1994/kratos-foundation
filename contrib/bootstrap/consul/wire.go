...
var ProviderSet = wire.NewSet(NewSpec, NewRemoteConfigPathsProvider)

// NewRemoteConfigPathsProvider 提供默认的八层 Consul 配置路径生成函数。
func NewRemoteConfigPathsProvider() bootstrap.RemoteConfigPathsProvider {
	return remoteConfigPaths
}

// remoteConfigPaths 保留八层初次加载顺序，后加载的路径覆盖前面的同名配置项。
