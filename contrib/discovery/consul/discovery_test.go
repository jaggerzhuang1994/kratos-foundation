...
func TestMultiDatacenterQueriesDoNotMixBlockingIndices(t *testing.T) {
	...
	d, err := NewDiscovery(testLogger(t), testconfig.New(t, "discovery", &config_pb.Discovery{Dc: config_pb.DC_MULTI.Enum(), Timeout: durationpb.New(time.Second)}), client)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMultiDatacenterCatalogHonorsCancellation(t *testing.T) {
	...
	d, err := NewDiscovery(testLogger(t), testconfig.New(t, "discovery", &config_pb.Discovery{Dc: config_pb.DC_MULTI.Enum(), Timeout: durationpb.New(40 * time.Millisecond)}), client)
	if err != nil {
		t.Fatal(err)
	}
