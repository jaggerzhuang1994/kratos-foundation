...
在仓库根目录执行 `make test-external`。只验证功能和竞态时运行 `make test-components-external`（脚本 `--functional-only` 模式），仍执行 GORM MySQL 用例，但省略 benchmark、pprof 和容器 stats；两者共用隔离创建与清理流程。需要 Docker Engine、Docker Compose v2、根 go.mod 对应的 Go 工具链；首次可能下载 compose.yaml 中的固定版本镜像。不需要现有数据库、云凭据或额外 Go 依赖。
...
| MySQL | Foundation 驱动真实建连、事务回滚、截止时间取消慢查询 | 不覆盖业务 schema、复杂 SQL、数据库主从切换 |
| Redis | 任务队列 Lua 延迟/重试/租约回收和旧 token 拒绝；两客户端互斥、跨原始 TTL 续租与释放；连接 kill/重连/关闭 | 不是 Redis 节点故障、主从切换或网络分区测试 |
...
Database 核心适配器提供 Repo 契约和 TaskRecord；可选默认 GORM Repo 的 MySQL 行为由下述测试验证，业务自定义 Repo 的原子领取、迁移和持久化集成测试仍由业务所属包负责。

默认 GORM Queue Repo 由 `make test-external` 通过 `FOUNDATION_TEST_QUEUE_MYSQL_DSN` 单独运行同一套仓储测试，结果写入 `queue-gorm.txt`；覆盖自定义模型字段、同事务提交/回滚、并发条件领取、旧 token 和同 ID 重建，测试只操作自己创建的 queue_test_tasks 表。自定义 Repo 仍需业务自己的契约测试。

Queue Stats 的真实 Redis Lua 校验由同一 `make test-external` 入口包含（TestStatsRedis）；覆盖状态计数、最老就绪年龄与超过1000候选后的年龄不可用行为。

新增 Kafka 消息往返/提交恢复、Redis 常见命令与订阅、Redis Lock 租约边界的场景见[核心组件集成用例](../../pkg/INTEGRATION_TESTS.md#扩展模块与常见边界)。功能模式保留 `functional.txt`、`queue-gorm.txt`、`services.txt`、`toolchain.txt`、`services.log` 和 `cleanup.log`。
