...
func (m *Conf) validate(all bool) error {
	...
	if m.Some != nil {
		...
		if all {
			switch v := interface{}(m.GetSome()).(type) {
		...
		} else if v, ok := interface{}(m.GetSome()).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
...
// Validate checks the field values on Some with the rules defined in the proto
// definition for this message. If any rules are violated, the first error
// encountered is returned, or nil if there are no violations.
func (m *Some) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on Some with the rules defined in the
// proto definition for this message. If any rules are violated, the result is
// a list of violation errors wrapped in SomeMultiError, or nil if none found.
func (m *Some) ValidateAll() error {
	return m.validate(true)
}

func (m *Some) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return SomeMultiError(errors)
	}

	return nil
}

// SomeMultiError is an error wrapping multiple validation errors returned by
// Some.ValidateAll() if the designated constraints aren't met.
type SomeMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m SomeMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m SomeMultiError) AllErrors() []error { return m }

// SomeValidationError is the validation error returned by Some.Validate if the
// designated constraints aren't met.
...
// Field function returns field value.
func (e SomeValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e SomeValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e SomeValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e SomeValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e SomeValidationError) ErrorName() string { return "SomeValidationError" }
...
var _ error = SomeValidationError{}
