...
const (
	errorsPackage = protogen.GoImportPath("github.com/jaggerzhuang1994/kratos-foundation/pkg/errors")
...
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P("// Code generated by protoc-gen-kratos-foundation-errors. DO NOT EDIT.")
	g.P()
...
func genErrorsReason(_ *protogen.Plugin, _ *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	defaultCodeOption := proto.GetExtension(enum.Desc.Options(), errors.E_DefaultCode)
	defaultCode := 500
	if ok := defaultCodeOption.(int32); ok != 0 {
		defaultCode = int(ok)
	}
	...
	for _, v := range enum.Values {
		...
		// 注释去掉签名的//
		commentLiteral := strings.TrimLeft(comment, "/ ")
		commentLiteral = strings.Trim(commentLiteral, " \n")

		hasFmt := false
		if strings.Contains(commentLiteral, "%") {
			// 如果包含%，则去除 %% 和 %空格，看是否还有包含 %，去除最后面的%
			hasFmt = strings.Contains(strings.TrimRight(strings.ReplaceAll(strings.ReplaceAll(commentLiteral, "%%", ""), "% ", ""), "%"), "%")
		}

		// 注释包一个双引号，可以直接在模板里输出使用
		commentLiteralJson, _ := json.Marshal(commentLiteral)
		err := &errorInfo{
			...
			HTTPCode:       enumCode,
			Comment:        comment,
			CommentLiteral: string(commentLiteralJson),
