...
func TestNewMapsArrayItemsUsingDraft04Keywords(t *testing.T) {
...
func TestMergePreservesExternalRootConstraints(t *testing.T) {
	...
	if external.Ref != "" || len(external.AllOf) == 0 || external.AllOf[0].Ref != "#/definitions/External" || external.ID == "" || external.Type != "object" || len(external.Required) != 1 || external.Version != "" || external.Definitions == nil {
...
func TestMergeRejectsIncompatibleDraftWithAccurateType(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "draft_04.Schema") {
		t.Fatalf("Merge() error = %v", err)
	}
}
