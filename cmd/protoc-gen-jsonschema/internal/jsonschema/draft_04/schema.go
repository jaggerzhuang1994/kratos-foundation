...
type Schema struct {
	...
	// Maximum 为数值上限；是否包含边界由 ExclusiveMaximum 决定，nil 表示未设置。
	Maximum *float64 `json:"maximum,omitempty"`
	// ExclusiveMaximum 为 true 时排除 Maximum 边界；nil 表示未设置。
	ExclusiveMaximum *bool `json:"exclusiveMaximum,omitempty"`
	// Minimum 为数值下限；是否包含边界由 ExclusiveMinimum 决定，nil 表示未设置。
	Minimum *float64 `json:"minimum,omitempty"`
	// ExclusiveMinimum 为 true 时排除 Minimum 边界；nil 表示未设置。
...
func (s *Schema) Merge(draft jsonschema.Draft) error {
	...
	if !ok {
		return fmt.Errorf("draft is not a draft_04.Schema")
	}
