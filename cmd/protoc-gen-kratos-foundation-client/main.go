...
type pkgSummary struct {
	...
	// 这里直接收集 serviceDesc，就能拿到 ServiceName
...
func generateWireFile(gen *protogen.Plugin, sum *pkgSummary) {
	// 文件名随便，你想叫 wire.go 也 OK，
	// 但建议带点后缀避免和手写文件冲突，比如 wire_client_gen.go。
	...
	for _, svc := range sum.Services {
		// 按你现在的规范，构造函数是 New<ServiceName>ClientImpl
