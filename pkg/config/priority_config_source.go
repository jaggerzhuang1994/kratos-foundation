...
type priorityConfigSource struct {
	sources []Source
	cached  map[Source][]*config.KeyValue
	lock    sync.Mutex
}

func NewPriorityConfigSource(sources []Source) Source {
	return &priorityConfigSource{
		sources,
		map[Source][]*config.KeyValue{},
		sync.Mutex{},
	}
}
...
func (p *priorityConfigSource) Watch() (config.Watcher, error) {
	...
	w := &watcher{
		...
		ctx:      ctx,
		cancel:   cancel,
		watchers: make(map[Source]config.Watcher, len(p.sources)),
	}
...
type watcher struct {
	...
	watchers map[Source]config.Watcher
}
