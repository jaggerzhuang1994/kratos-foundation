...
func TestProducerPreparesCopyAndInjectsTraceContext(t *testing.T) {
	...
	producer, err := NewManagedProducer("orders.created", raw, observability)
	if err != nil {
		t.Fatal(err)
	}
	...
	if countSpan(spans, "kafka.publish") != 1 {
...
func TestProducerBatchRejectsInvalidInputBeforePublishing(t *testing.T) {
	raw := newRecordingProducer()
	producer, err := NewManagedProducer("orders.created", raw, newDisabledTestObservability(t))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProducerPreservesDriverErrorAndUsesStableMetricLabels(t *testing.T) {
	...
	labels := gatheredQueueMetricLabels(t, observability.Metrics, "kafka_producer_messages_total")
	if labels["kafka_destination"] != "orders.created" || labels["kafka_result"] != "error" {
...
func TestProducerFailureLogExcludesPayloadAndHeaderValues(t *testing.T) {
	...
	written := logs.String()
	if !strings.Contains(written, "kafka.publish.failed") {
...
func TestProducerRejectionLogsWithCallerContextAtWarn(t *testing.T) {
	...
	logs := newRecordingLogger()
	observability.Logger = logs
	producer, err := NewManagedProducer("orders.created", newRecordingProducer(), observability)
	if err != nil {
		t.Fatal(err)
	}
	...
	if written := logs.String(); !strings.Contains(written, "kafka.publish.rejected") ||
...
func TestProducerFailureLogsWithActiveProducerSpanContextAtError(t *testing.T) {
	...
	logs := newRecordingLogger()
	observability.Logger = logs
	producer, err := NewManagedProducer(
		"orders.created",
	...
	if written := logs.String(); !strings.Contains(written, "kafka.publish.failed") {
		t.Fatalf("failure event missing from log %q", written)
	}
}
