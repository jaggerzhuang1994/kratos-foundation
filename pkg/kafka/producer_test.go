...
func TestProducerPublishEncodesIndependentRecordAndWrapsFailure(t *testing.T) {
	...
	message := &Message{
		...
		Key:       []byte("key"),
		Body:      []byte("body"),
		Headers:   []Header{{Key: messageIDHeader, Value: []byte("stale")}, {Key: "trace", Value: []byte("trace-1")}},
	...
	err = failingValue.Publish(context.Background(), &Message{ID: "message-2"})
...
func TestProducerPublishRejectsInvalidStateAndInput(t *testing.T) {
	...
	if err := producer.Publish(context.Background(), &Message{}); err == nil || !strings.Contains(err.Error(), "not initialized") {
...
func TestProducerPublishBatchAlignsOutOfOrderFailuresToInputs(t *testing.T) {
	...
	messages := []*Message{{ID: "one"}, {ID: "two"}, {ID: "three"}}
...
func TestProducerPublishBatchHandlesEmptyNilAndSuccessfulInputs(t *testing.T) {
	...
	if err := nilProducer.PublishBatch(context.Background(), []*Message{{ID: "one"}}); err == nil {
	...
	if err := value.PublishBatch(context.Background(), []*Message{{ID: "one"}, nil}); err == nil || !strings.Contains(err.Error(), "message 1 is nil") {
		t.Fatalf("nil batch message error = %v", err)
	}
	if err := value.PublishBatch(context.Background(), []*Message{{ID: "one"}, {ID: "two"}}); err != nil {
		t.Fatal(err)
	}
...
func TestCollectBatchFailuresRejectsMalformedProducerResults(t *testing.T) {
	messages := []*Message{{ID: "one"}, {ID: "two"}}
