...
func TestTelemetryRecordsAttemptOutcomesAndFinalClassifications(t *testing.T) {
	...
	for _, event := range spans[0].Events {
		if len(event.Name) >= len("kafka.") && event.Name[:len("kafka.")] == "kafka." {
			queueEvents = append(queueEvents, event.Name)
		}
	}
	if len(queueEvents) != 3 || queueEvents[0] != "kafka.consume.attempt" || queueEvents[1] != "kafka.consume.attempt" || queueEvents[2] != "kafka.consume.permanent" {
	...
	for _, result := range []string{"success", "error"} {
		...
		if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_attempts_total", labels); sample.counter != 1 {
		...
		if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_attempt_duration_seconds", labels); sample.histogramCount != 1 || sample.histogramSum != wantDuration {
...
func TestTelemetryRecordsMessageRetryDeadLetterRuntimeAndProducer(t *testing.T) {
	...
	if len(events) != 3 || events[0].Name != "kafka.consume.retry" || events[1].Name != "kafka.consume.dead_lettered" || events[2].Name != "kafka.dead_letter.failed" {
	...
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_messages_total", map[string]string{
		"kafka_destination": "orders", "kafka_consumer": "worker", "kafka_result": "success",
	}); sample.counter != 1 {
		t.Fatalf("message counter = %v, want 1", sample.counter)
	}
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_message_duration_seconds", map[string]string{
		"kafka_destination": "orders", "kafka_consumer": "worker", "kafka_result": "success",
	...
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_retries_total", map[string]string{
		"kafka_destination": consumerLabels["kafka_destination"], "kafka_consumer": consumerLabels["kafka_consumer"], "kafka_result": "scheduled",
	}); sample.counter != 1 {
		t.Fatalf("retry counter = %v, want 1", sample.counter)
	}
	for _, result := range []string{"success", "error"} {
		if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_dead_letters_total", map[string]string{
			"kafka_destination": "orders", "kafka_consumer": "worker", "kafka_result": result,
		}); sample.counter != 1 {
			t.Fatalf("%s dead-letter counter = %v, want 1", result, sample.counter)
		}
	}
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_consumer_runtime_failures_total", map[string]string{
		"kafka_destination": "orders", "kafka_consumer": "worker", "kafka_result": "error",
	}); sample.counter != 1 {
		t.Fatalf("runtime failure counter = %v, want 1", sample.counter)
	}
	producerLabels := map[string]string{
		"kafka_destination": "orders", "kafka_operation": "publish", "kafka_result": "success",
	}
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_producer_messages_total", producerLabels); sample.counter != 2 {
		t.Fatalf("producer counter = %v, want 2", sample.counter)
	}
	if sample := findTelemetryMetricSample(t, metricProvider, "kafka_producer_duration_seconds", producerLabels); sample.histogramCount != 1 || sample.histogramSum != 1 {
