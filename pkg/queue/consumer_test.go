...
func TestConsumerRuntimeRetriesThenDeadLettersAndAcknowledges(t *testing.T) {
	...
	written := logs.String()
	if !strings.Contains(written, "queue.consume.dead_lettered") || strings.Contains(written, "dead-letter-secret") {
		t.Fatalf("dead-letter log = %q", written)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_messages_total", map[string]string{"queue_result": "dead_lettered"}); sample.counter != 1 {
		t.Fatalf("final message count = %v", sample.counter)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_message_duration_seconds", map[string]string{"queue_result": "dead_lettered"}); sample.histogramCount != 1 {
		t.Fatalf("final duration count = %d", sample.histogramCount)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_dead_letters_total", map[string]string{"queue_result": "success"}); sample.counter != 1 {
...
func TestConsumerRuntimeDeadLetterFailureLeavesOriginalUnacknowledged(t *testing.T) {
	...
	written := logs.String()
	if !strings.Contains(written, "queue.dead_letter.failed") || strings.Contains(written, "dead-letter-secret") {
		t.Fatalf("dead-letter failure log = %q", written)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_dead_letters_total", map[string]string{"queue_result": "error"}); sample.counter != 1 {
		t.Fatalf("dead-letter failure count = %v", sample.counter)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_runtime_failures_total", map[string]string{"queue_result": "error"}); sample.counter != 1 {
...
func TestConsumerRuntimeExtractsPropagationAndRetriesInsideOneConsumerSpan(t *testing.T) {
	...
	for index, span := range spans {
		if span.Name == "queue.consume" {
			consumeIndex = index
			consumeCount++
		}
	}
	if consumeCount != 1 {
		t.Fatalf("queue.consume spans = %d, want 1", consumeCount)
	}
	...
	for _, event := range span.Events {
		switch event.Name {
		case "queue.consume.attempt":
			attemptEvents++
		case "queue.consume.retry":
			retryEvents++
		}
	}
...
func TestConsumerRuntimeRecordsControlledFinalClassificationEvents(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			wantEvent := "queue.consume." + test.wantClass
			found := false
			for _, event := range span.Events {
				...
				for _, field := range event.Attributes {
					if string(field.Key) == "queue.attempts" {
						attemptsFound = true
				...
				if !attemptsFound {
					t.Fatalf("classification event = %#v, want queue.attempts", event)
				}
			}
			if !found {
...
func TestConsumerRuntimeRetryObservabilityUsesStableNonSensitiveFields(t *testing.T) {
	...
	written := logs.String()
	if !strings.Contains(written, "queue.consume.retry") || strings.Contains(written, "body-secret") || strings.Contains(written, "header-secret") || strings.Contains(written, "trace-secret") {
		t.Fatalf("retry log = %q", written)
	}
	labels := gatheredQueueMetricLabels(t, observability.Metrics, "queue_consumer_attempts_total")
	if labels["queue_destination"] != "orders.created" || labels["queue_consumer"] != "order-created" {
...
func TestConsumerRuntimePermanentFailureLogsOnlyControlledFieldsAndRunsOnce(t *testing.T) {
	...
	written := logs.String()
	if !strings.Contains(written, "queue.consume.failed") || strings.Contains(written, "body-secret") || strings.Contains(written, "header-secret") || strings.Contains(written, "trace-secret") {
		t.Fatalf("failure log = %q", written)
	}
	if !strings.Contains(written, "queue.consumer.failed") {
		t.Fatalf("runtime failure log = %q", written)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_messages_total", map[string]string{"queue_result": "error"}); sample.counter != 1 {
		t.Fatalf("final failure count = %v", sample.counter)
	}
	if sample := findQueueMetricSample(t, observability.Metrics, "queue_consumer_message_duration_seconds", map[string]string{"queue_result": "error"}); sample.histogramCount != 1 {
