...
// NewProducer 将外部 Producer 包装为保留消息所有权且具备统一观测能力的 Producer。
func NewProducer(name string, producer Producer, observability Observability) (Producer, error) {
...
func (p *managedProducer) publish(
	...
	spanCtx, span := p.telemetry.Tracer().Start(ctx, "queue.publish", trace.WithSpanKind(trace.SpanKindProducer))
	defer span.End()
	span.SetAttributes(
...
func (p *managedProducer) rejected(ctx context.Context, operation string, started time.Time) {
	...
	if p.log != nil {
		p.log.WithContext(ctx).Warnw(
			"event", "queue.publish.rejected",
			"queue.destination", p.destination,
		)
	}
}
