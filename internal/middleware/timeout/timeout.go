...
type Config = *config_pb.Middleware_Timeout

func Server(config Config) middleware.Middleware {
	return makeMiddleware(transport.FromServerContext, time.Second, config)
}

func Client(config Config) middleware.Middleware {
	return makeMiddleware(transport.FromClientContext, 2*time.Second, config)
}
