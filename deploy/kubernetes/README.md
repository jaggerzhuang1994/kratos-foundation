...
node-exporter 的 env/cluster 必须和应用 ServiceMonitor 一致，node 必须为真实 Kubernetes Node 名。仅设置 Prometheus externalLabels 不会给本地存储中的每条序列补齐这些标签；本模板要求在目标 relabeling 中补齐。
...
Pod 维度的应用图显示的是所选 Pod 中应用进程指标。完整容器 CPU/内存、CPU throttling、重启和 Deployment 副本状态请结合 kube-prometheus-stack 自带 Kubernetes 面板（kubelet/cAdvisor、kube-state-metrics）；本模板不把进程 RSS 标成容器 working set。

## 4. 导入 Dashboard 和告警

可以直接在 Grafana 导入 [应用总览](../observability/grafana/dashboards/foundation.json) 和 [组件明细](../observability/grafana/dashboards/foundation-components.json)，选择已有 Prometheus 数据源。也可以使用 sidecar；生成的 ConfigMap 同时包含这两份面板：
...
ConfigMap 必须位于 Grafana sidecar 搜索的命名空间，标签需匹配 sidecar 的 label/labelValue；默认示例为 `grafana_dashboard=1`。Dashboard 数据源变量可选择已有 Prometheus；JSON 内默认 UID `foundation-prometheus` 来自本地 Compose，现有 Grafana 若没有这个 UID，打开后选择实际数据源并保存业务副本，或在源 JSON 中调整默认值再生成。
...
flowchart TD
    ...
    K --> I
    J -- 是 --> L([在 Grafana 按 App Pod Node 验证真实数据])
```
...
应用ServiceMonitor新增target标签（App / Node / Pod / 地址），面板默认实例明细使用该标签；升级后新样本生效，旧历史样本仍可按instance查看。
