...
## FoundationConfigRejected

检查配置发布记录、Manager/源解码错误以及组件的配置更新日志。Manager 接受快照不等于所有组件应用成功；构造期字段需要重启，非法组件组合可能保留旧策略。按 [配置说明](../../../pkg/config/README.md) 和相关包 README 核对支持范围。

比较字段名称、类型、Duration 格式和禁用字段，不在日志或工单中粘贴凭据。修正配置后确认拒绝计数不再增加，并通过实际行为核对期望生效；告警窗口结束不代表配置已修好。

## FoundationConfigSubscriptionOverloaded

检查慢订阅回调和待处理队列，定位未及时响应取消的外部调用。过载会终止对应订阅；计数不再增长或告警恢复，不等于订阅恢复。修复回调后按应用生命周期重新订阅或通过已批准的重启流程恢复，确认订阅接受状态和实际行为。
